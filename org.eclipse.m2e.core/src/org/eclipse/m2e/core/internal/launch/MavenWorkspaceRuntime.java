  ...
  public void createLauncherConfiguration(IMavenLauncherConfiguration collector, IProgressMonitor monitor)
    ...
    if(maven != null) {
      ...
      if(launcherArtifact != null) {
        ...
        addArtifact(collector, launcherArtifact, projectRegistry);
      }
    }
  }

  private void addArtifact(IMavenLauncherConfiguration collector, Artifact artifact, IMavenProjectRegistry registry)
      throws CoreException {
    IMavenProjectFacade facade = registry.getMavenProject(artifact.getGroupId(), artifact.getArtifactId(),
        artifact.getVersion());
    if(facade != null) {
